#!/usr/bin/env python3
"""
Simple Icon Accessibility Fixer for Kristina's Academic Success Dashboard
...
class SimpleIconAccessibilityFixer:
    ...
    def add_missing_aria_labels(self, content, file_path):
        ...
        # Find all icons and add aria-labels if missing
        for icon_name, aria_label in icon_labels.items():
            # Pattern to find icons without aria-label
            pattern = rf'<i data-lucide="{icon_name}"([^>]*?)(?<!aria-label="[^"]*")>'
            
            def add_aria_label(match):
                attributes = match.group(1)
                # Check if aria-label already exists
                if 'aria-label=' not in attributes:
                    return f'<i data-lucide="{icon_name}"{attributes} aria-label="{aria_label}">'
    ...
    def run(self):
        ...
        print("🎯 Starting simple icon accessibility fixes...")
        print("=" * 60)
...
if __name__ == "__main__":
    fixer = SimpleIconAccessibilityFixer()
    fixer.run()
